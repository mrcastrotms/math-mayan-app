...
if (!uid || !email) {
  console.error(
    'Missing required args. Example: --uid=UID --email=teacher@example.com --name="Teacher Name"',
  );
  process.exit(1);
}
...
if (!admin.apps.length) {
  ...
  if (sdkJson) {
    ...
    console.error(
      "No Firebase admin credentials found. Set FIREBASE_ADMIN_SDK env var or create serviceAccount.json or set GOOGLE_APPLICATION_CREDENTIALS.",
    );
    process.exit(1);
  }
}
